...
public Result<Widget> AdjustQuantity(int delta)
{
    ...
    return next.RaiseEvent(new WidgetQuantityChanged(Id));
}
```
...
`AggregateRoot` is an `abstract record` whose outbox is an `init`-only `IReadOnlyList<IDomainEvent> DomainEvents` (no mutable `List`, no copy-constructor). Raising is functional — `protected TSelf RaiseEvent(IDomainEvent)` returns `(TSelf)(this with { DomainEvents = [.. DomainEvents, evt] })`; because the list is immutable, a plain `with` safely shares it, so a transition's new instance carries the prior's still-pending events forward. The aggregate can't clear itself in place, so `DequeueEvents()` is also functional: it returns `(IReadOnlyList<IDomainEvent> Events, AggregateRoot Cleared)` — the pending events plus a cleared copy.

The self type and the id type are **type parameters of the base, not of each method**: a concrete aggregate derives from **`AggregateRoot<TSelf, TId>`** (`public sealed record Widget : AggregateRoot<Widget, WidgetId>`) and overrides `public abstract TId Id { get; }` with a get-only auto-property assigned in its constructor — the record copy constructor copies backing fields, so `with` and the cleared copy carry the id forward. A non-generic `AggregateRoot` stays underneath as the untyped handle infrastructure holds (`TrackedAggregates`, `DequeueEvents`). Because the base knows both the self type and the identity, it supplies what every aggregate would otherwise hand-write: a `RaiseEvent(evt)` returning the concrete type (no `<Widget>` at the call site) and a `sealed` `HasSameIdentity`.

Because aggregates are records, identity tracking can't rely on reference or value equality (a transition yields a new, value-distinct instance of the same aggregate). `AggregateRoot` exposes an abstract **`HasSameIdentity(AggregateRoot)`**, implemented once by the generic base as *same self type + same id* (`other is TSelf o && EqualityComparer<TId>.Default.Equals(Id, o.Id)`); `TrackedAggregates.Track` tracks by it and **replaces** a prior instance of the same aggregate with the latest. `TrackedAggregates.DequeueEvents` calls each tracked aggregate's `DequeueEvents()` and **swaps in the returned cleared copy** — so each event dispatches once and a re-drain after a handler cascade sees only freshly raised events. `DomainEventDrain` is unchanged (drains before commit, loops for cascades).
...
- **`ACME004`** — guards the self-type pattern at a **call site**: when invoking a generic method with a `TSelf` type parameter, the supplied type argument must be the **calling type** — `Greeting` calling `SomeSelfTyped<Widget>()` (an `InvalidCastException` at runtime) is a build error.
...
- **`ACME009`** — the same convention at a **declaration**: a type whose base type (anywhere up its chain) or directly implemented interface is generic in a parameter named `TSelf` must have **itself** as that argument. The constraint can't express this — `record Gadget : AggregateRoot<Widget, WidgetId>` satisfies `where TSelf : AggregateRoot<TSelf, TId>` and compiles, then throws `InvalidCastException` on the first `RaiseEvent`. The chain matters because an intermediate that *closes* `TSelf` on itself (`abstract record Audited : AggregateRoot<Audited, WidgetId>`) leaves every leaf below it with `TSelf = Audited`: two leaves sharing an id would then compare as the same aggregate and `TrackedAggregates` would drop one's pending events. An intermediate must forward `TSelf` (stay generic in it) instead. ACME009 **complements** ACME004 rather than replacing it: `RaiseEvent` is no longer generic, so it is covered here, while ACME004 still guards every other generic `TSelf` method call.

A further rule guards the persistence boundary in `*.Infrastructure` projects (opt-in via `EnforceEfEntityBoundary`):
